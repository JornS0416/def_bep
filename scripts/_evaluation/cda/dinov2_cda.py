"""
DINOv2 — Cross-Dataset Analysis (CDA)
======================================
...
Feature aggregation
-------------------
DINOv2 features have shape (81, 768) per clip (CLS token per frame).
For cross-dataset evaluation with a linear probe, the temporal sequence is
collapsed to a single (768,) vector via mean pooling over the 81 frames —
identical to dinov2_lodo.py, and for the same reason: training a BiLSTM
across heterogeneous cohorts (different cadence, FPS, recording setups)
is not meaningful, whereas a linear probe on pooled features is directly
comparable across cohorts and to the other models' CDA results.

The within-dataset LOSO (dinov2_loso.py) uses a BiLSTM and remains the
primary within-dataset evaluation; this script is the cross-dataset one.

See videomae_cda.py for the full CDA rationale (why CDA vs. LODO, why no
diagonal, why a single train/test split per pair, why the probe is fit
once per source, etc.)

Usage
-----
    cd main_project
    python scripts/_evaluation/cda/dinov2_cda.py
"""
...
RESULTS_DIR   = output_root() / "results" / "cda" / "dinov2"
...
def run_cda(records: list, datasets: list) -> dict:
    ...
    for source in datasets:
        ...
        X_train = np.stack([r["feature"].mean(axis=0) for r in train_rows])
        ...
        for target in datasets:
            ...
            X_test   = np.stack([r["feature"].mean(axis=0) for r in test_rows])
...
def main() -> None:
    ...
    output = {
        ...
        "note":       "Feature aggregation: mean over (81, 768) → (768,), "
                      "identical to dinov2_lodo.py. Within-dataset LOSO uses "
                      "BiLSTM (dinov2_loso.py).",
        "datasets":   DATASETS,
        "pairs":      result["pairs"],
    }

    json_path = RESULTS_DIR / f"results_dinov2_cda_{run_ts}.json"
    with open(json_path, "w") as f:
        json.dump(output, f, indent=2)
    log.info("Saved to %s", json_path)

    for metric, mat in result["matrices"].items():
        mat_path = RESULTS_DIR / f"results_dinov2_cda_matrix_{metric}_{run_ts}.csv"
        mat.to_csv(mat_path)
