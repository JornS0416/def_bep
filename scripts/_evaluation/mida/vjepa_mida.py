"""
V-JEPA — Multi-dataset In-domain Adaptation (MIDA) Evaluation
===============================================================
Structurally identical to videomae_mida.py.
Only differences: feature file, embed dim (1024 vs 3072), results dir.

See videomae_mida.py for the full rationale (verbatim Care-PD MIDA protocol,
why a single-stage fit is mathematically equivalent to "fine-tune from a
LODO checkpoint" for convex linear probes, why no domain-balancing sample
weights are needed, and the LODO/MIDA/LOSO comparison frame).

Usage
-----
    cd main_project
    python scripts/_evaluation/mida/vjepa_mida.py
"""
...
RESULTS_DIR   = output_root() / "results" / "mida" / "vjepa"
...
def run_mida_dataset(
    ...
    """MIDA for one target dataset. See videomae_mida.py for full documentation."""
...
def main() -> None:
    ...
    output = {
        ...
        "model":      "V-JEPA ViT-L/16 (facebookresearch/jepa), frozen",
        ...
        "rationale": {
            ...
            "single_stage_fit":   "Convex linear probes converge to the same "
                                  "solution regardless of init — fitting "
                                  "directly on (target_train ∪ auxiliary) is "
            ...
            "no_sample_weighting":"Each LOSO fold's target-train split already "
                                  "contains ~(n-1)/n of the cohort — comparable "
                                  "in scale to the auxiliary pool.",
    ...
    json_path = RESULTS_DIR / f"results_vjepa_mida_{run_ts}.json"
    ...
    for ds, res in dataset_results.items():
        if res.get("cm_df") is not None:
            cm_path = RESULTS_DIR / f"results_vjepa_mida_{ds}_cm_{run_ts}.csv"
