"""
VideoMAE — Leave-One-Subject-Out (LOSO) Evaluation
====================================================
Evaluates frozen VideoMAE multi-layer features using LOSO cross-validation,
applied per dataset separately. Produces results directly comparable to the
Care-PD paper baseline (which also uses LOSO per dataset).

Input:  videomae_features_multilayer.pkl — (3072,) mean-pool vectors per clip
Output: outputs/runs/<run>/results/loso/videomae/results_videomae_loso_{ts}.json

─────────────────────────────────────────────────────────────────────────────
WHY LOSO INSTEAD OF 5-FOLD
─────────────────────────────────────────────────────────────────────────────

The 5-fold evaluation in scripts/videomae/ordinal_regression.py is a
computationally efficient approximation. The Care-PD paper (sec. 4.2) uses
Leave-One-Subject-Out (LOSO) as its within-dataset evaluation protocol,
because it:

1. Maximises training data per fold (all subjects except one).
2. Provides the most pessimistic, realistic estimate of generalisation —
   each test subject is completely unseen during training.
3. Is directly comparable to the Care-PD baseline numbers.

LOSO is implemented here via GroupKFold(n_splits=n_subjects), where
groups = subject IDs. Each fold uses exactly one subject as test set.

─────────────────────────────────────────────────────────────────────────────
WHY PER DATASET (NOT COMBINED)
─────────────────────────────────────────────────────────────────────────────

The Care-PD paper evaluates per dataset. Combining datasets introduces
domain shift between cohorts (different recording setups, demographics,
UPDRS distributions), which conflates within-dataset generalisation with
cross-dataset generalisation. Evaluating per dataset:

1. Isolates each cohort's signal.
2. Matches the Care-PD comparison target.
3. Exposes dataset-specific performance differences that a combined
   evaluation would obscure.

─────────────────────────────────────────────────────────────────────────────
FIXED C = 1.0 (NO NESTED CV TUNING)
─────────────────────────────────────────────────────────────────────────────

The 5-fold script (ordinal_regression.py) uses nested CV GridSearchCV to
tune C per fold. For LOSO this is omitted for two reasons:

1. LOSO is already the strictest evaluation protocol — additional tuning
   complexity adds minimal benefit and makes the pipeline harder to compare
   directly with the Care-PD baseline (which also uses a simple linear probe).
2. With small datasets like 3DGait (210 clips total, ~4 clips/subject),
   inner CV within a LOSO fold would have too little data to be reliable.

C=1.0 is the sklearn default and a well-validated choice for standardised
high-dimensional features. It is consistent with the Care-PD paper's
lightweight probe philosophy.

─────────────────────────────────────────────────────────────────────────────
DATASET SIZES AND RELIABILITY NOTES
─────────────────────────────────────────────────────────────────────────────

PD-GaM : 30 subjects, ~80 clips/subject  → 30 LOSO folds. Reliable.
BMClab  : 23 subjects, ~193 clips/subject → 23 LOSO folds. Reliable.
3DGait  : 43 subjects, ~4 clips/subject   → 43 LOSO folds. NOISY.
          With only 2-16 clips per subject, majority voting operates on
          very few predictions per walk. Metrics for 3DGait should be
          interpreted with caution.

Usage:
    cd main_project
    python scripts/_evaluation/loso/videomae_loso.py

Prerequisite: run scripts/videomae/extract_features.py first to produce
              videomae_features_multilayer.pkl.
"""
...
RESULTS_DIR   = output_root() / "results" / "loso" / "videomae"

DATASETS     = ["PD-GaM", "BMClab", "3DGait", "T-SDU-PD"]
LR_C         = 1.0      # fixed — see rationale in module docstring
RIDGE_ALPHA  = 1.0
RANDOM_STATE = 42
UPDRS_MIN    = 0
UPDRS_MAX    = 3
...
def clip_and_round(x: np.ndarray) -> np.ndarray:
...
def run_loso_dataset(
    ...
    """
    Run LOSO evaluation for one dataset.

    GroupKFold(n_splits=n_subjects): each fold has exactly one subject
    as test set. No stratification — inherent to LOSO with 1 test subject.

    Returns dict with per-fold results, summary stats, and confusion matrix.
    """
    ...
    # Accumulate all walk-level predictions for aggregate confusion matrix
    # AND for a single pooled Spearman correlation (see note below the loop —
    # per-fold Spearman is mathematically undefined in LOSO and was producing
    # NaN for every fold).
    all_walk_true = []
    ...
    #    `last_report_allfolds` report). `{metric}_mean` therefore holds the
    #    POOLED headline value; `{metric}_std` holds the between-subject SD of
    #    the per-fold values (Care-PD Appendix D.4 variability), NOT a fold mean.
...
def main() -> None:
    ...
    for dataset in DATASETS:
        ...
        if not rows:
            log.warning("No records for dataset %s — skipping.", dataset)
            continue
        ...
        result = run_loso_dataset(X, y, groups, walk_ids, dataset)
    ...
    # Strip cm_df before JSON serialisation; save separately as CSV
    output = {
        ...
        "rationale": {
            ...
            "fixed_C":    f"C={LR_C} fixed to keep probe minimal and comparable to "
                          "Care-PD baseline. Nested CV tuning omitted: LOSO is already "
                          "the strictest protocol, and 3DGait is too small for inner CV.",
        },
        "datasets": {
    ...
    json_path = RESULTS_DIR / f"results_videomae_loso_{run_ts}.json"
    ...
    for ds, res in dataset_results.items():
        if res.get("cm_df") is not None:
            cm_path = RESULTS_DIR / f"results_videomae_loso_{ds}_cm_{run_ts}.csv"
