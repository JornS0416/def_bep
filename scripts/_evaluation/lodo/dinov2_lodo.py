"""
DINOv2 — Leave-One-Dataset-Out (LODO) Evaluation
==================================================
...
For cross-dataset LODO with a linear probe, the temporal sequence is
collapsed to a single (768,) vector via mean pooling over the 81 frames.
This matches the linear probe philosophy of the Care-PD paper and avoids
training a BiLSTM across heterogeneous datasets (different cadence, FPS).

The within-dataset LOSO (dinov2_loso.py) uses a BiLSTM and is the primary
within-dataset evaluation. This LODO script is the cross-dataset evaluation.

See videomae_lodo.py for full rationale on LODO design choices.

Usage
-----
    cd main_project
    python scripts/_evaluation/lodo/dinov2_lodo.py
"""
...
RESULTS_DIR   = output_root() / "results" / "lodo" / "dinov2"
...
def run_lodo(records: list, datasets: list) -> dict:
    ...
    for held_out in datasets:
        ...
        # Mean-pool (81, 768) → (768,) for linear probe
...
def main() -> None:
    ...
    output = {
        ...
        "note":       "Feature aggregation: mean over (81, 768) → (768,). "
                      "Within-dataset LOSO uses BiLSTM (dinov2_loso.py).",
        "datasets":   DATASETS,
        "folds":      result["folds"],
        "summary":    result["summary"],
    }

    json_path = RESULTS_DIR / f"results_dinov2_lodo_{run_ts}.json"
    with open(json_path, "w") as f:
        json.dump(output, f, indent=2)
    log.info("Saved to %s", json_path)

    cm_path = RESULTS_DIR / f"results_dinov2_lodo_cm_{run_ts}.csv"
